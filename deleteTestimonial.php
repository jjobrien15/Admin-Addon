<?php
  ...
  try{
    $sql = "SELECT author, body FROM testimonials WHERE id = :id";
    $stmt = $pdo->prepare($sql);
    $stmt->bindValue(":id", $selectedTestimonial);
    $stmt->execute();
...
<div class="content">
  ...
  <p>Are you sure you want to delete  <?php echo $testimonial['author']; ?>'s testimonial?</p>
  <form method="POST">
    <input class="btn btn-success" type="submit" value="Yes"/>
    <a class="btn btn-danger" href="testimonials.php">No</a>
  </form>
  </div>
</div>
